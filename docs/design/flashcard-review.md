...
| イベント | 次回出題まで |
|----------|-------------|
| 間違えた（初回 / 再挑戦どちらも）| 翌日（24時間後）、intervalDays を 1 にリセット |
| 復習問題に正解した | intervalDays × 2 日後（上限 30 日） |
...
- 途中で間違えた場合 → intervalDays を 1 にリセットして翌日に再登録
- intervalDays は `review_queue` テーブルに保持する
...
CREATE TABLE review_queue (
  ...
  interval_days   INTEGER NOT NULL DEFAULT 1, -- スペースド・リペティション用インターバル（日数）
  UNIQUE (user_id, question_id)
);
```

- `next_review_at <= now()` の行が「今日以降に出題すべき問題」
- 同じ問題を再び間違えた場合は `wrong_count` をインクリメントし `next_review_at` を翌日に更新、`interval_days` を 1 にリセット（UPSERT）
- 復習問題に正解した場合は `interval_days` を 2 倍（上限 30）にして `next_review_at` を更新
...
if (!isCorrect) {
  ...
  await db.insert(reviewQueue).values({ ... nextReviewAt: now + 24h, intervalDays: 1 ... })
    .onConflictDoUpdate({ set: { nextReviewAt, intervalDays: 1, wrongCount: wrongCount + 1 } });
  out.isReviewRegistered = true;
} else {
  // 正解: review_queue に存在する場合のみ intervalDays を 2 倍（上限 30）にして nextReviewAt を更新
  const updated = await db.update(reviewQueue)
    .set({
      intervalDays: sql`MIN(intervalDays * 2, 30)`,
      nextReviewAt: sql`now + MIN(intervalDays * 2, 30) * dayMs`,
    })
    .where(userId === ... && questionId === reviewKeyId)
    .returning({ id, nextReviewAt });
  if (updated.length > 0) out.reviewNextAt = updated[0].nextReviewAt;
}
```
...
1. ユーザーが復習問題に回答（正解）
2. submitQuizAnswerAction（Server Action）が呼ばれる
3. サーバーが review_queue の intervalDays を 2 倍（上限 30 日）にして next_review_at を更新
4. レスポンスに reviewNextAt フィールドを含める
5. クライアントが「X 日後に再出題します」メッセージを動的に表示
```
...
- [x] SM-2 等のより精密なアルゴリズムへの移行 → intervalDays を 2 倍ずつ増やす指数バックオフ方式（上限 30 日）で実装済み
