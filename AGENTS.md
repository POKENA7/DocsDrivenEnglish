...
- 可能なら最低限 `pnpm run lint` / `pnpm run test:run` / `pnpm run format:check` を通す
...
- `pnpm run preview` は `wrangler dev --local --persist-to .wrangler/state` を起動します
...
- `pnpm run preview` が D1 で落ちる: `wrangler.toml` の `d1_databases.database_id` が未設定/誤りの可能性。`wrangler d1 create` 後に正しい ID を設定する
- ローカルDBを作り直したい: `frontend/.wrangler/state` を削除してから `pnpm run db:migrate:local`
